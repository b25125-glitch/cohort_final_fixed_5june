// /api/future.js — streaming + accumulate (Future Needs Explorer)
// Same reliability fix as /api/cohort. Mintel signals + cohort projection logic
// in the prompt are untouched — only the transport is changed.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!query || !sp) { res.status(400).json({ error: 'Missing query or sp' }); return; }
